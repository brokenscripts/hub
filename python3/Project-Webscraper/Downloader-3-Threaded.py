...
def download_chapter(address):
    dlchapter = address.strip('/').split('-')[-1]
    print(f'Downloading Chapter {dlchapter}...')
    ...
    if not all_images:
        ...
        try:
            for i in all_images:
                ...
                res.raise_for_status()
                with open(os.path.join(MANGANAME, dlchapter, os.path.basename(image_url)), 'wb') as image_file:
...
def main():
    ...
    [task_queue.put(item) for item in pages]
